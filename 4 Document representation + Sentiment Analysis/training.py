...
def cleanLabeledComment(labeled_comment_path,labeled_comment_file):
    ...
    with io.open(stopwords_path+'/'+stopwords_name,encoding='utf-8') as f:
        line=f.readline()
        while line:
    ...
    label_comment=io.open(labeled_comment_path+'/'+labeled_comment_file,encoding='utf-8')
...
def allInput(sentiment_list,sentence_list,raw_word_list):
    ...
    '''
    # (1) Calculate the average sentence length K
    sentLength=len(raw_word_list)/len(sentence_list)+6
    print(sentLength)
    ...
    for i in range(len(sentence_list)):
        if len(sentence_list[i])>sentLength:
            # 1. 评论单词数>平均单词数 根据tf-idf分数排名，提取前K个关键词，压缩评论
    ...
    # (3) Concat all comment vectors
    flag=0
    #all_sentence_vec=np.zeros((int(round(sentLength+1)),numDimensions))
    all_sentence_vec=np.zeros((int(eval(str(Decimal(sentLength+1).quantize(Decimal('0'))))),numDimensions))
    ...
    #all_sentence_vec=all_sentence_vec.reshape(-1,int(round(sentLength+1)),numDimensions)
    all_sentence_vec=all_sentence_vec.reshape(-1,int(eval(str(Decimal(sentLength+1).quantize(Decimal('0'))))),numDimensions)
...
def CNN(lstmUnits,keepratio,data,kernel_size,numClasses):
    ...
    #embedding = tf.get_variable('embedding', [vocab_size, numDimensions])
    #embedding_inputs = tf.nn.embedding_lookup(embedding, data)
    ...
    print ("prediction_T",type(prediction))
    return prediction
...
def stacked_CNN(lstmUnits,keepratio,data,kernel_size,numClasses):
    ...
    conv = tf.layers.conv1d(gmp, lstmUnits, kernel_size2, name='conv1')
...
#    sentLength      = int(round(len(raw_word_list)/len(sentence_list)+1)+5)        # Sentence Length
    sentLength      = int(eval(str(Decimal(len(raw_word_list)/len(sentence_list)+1).quantize(Decimal('0'))))+6)        # Sentence Length
    # Num of forward and backward layer of Bi-LSTM250                         #
    num_layers      = 3                                                       # Layer Num
    ...
    #CNN Input
    #data=tf.placeholder(tf.int32,[None,sentLength])
    ...
    # (5) Define loss function and optimizer
    # Regularization term (if CNN, without that)
...
#################################################################################################                                   
        ...
        for epoch in range(training_epochs):
            ...
            if epoch % save_step ==0 and epoch!=0:
                #CNN
                model_name='stacked_CNN_CBOW'
                #model_name='StackedBLSTM_{}layers_CBOW'.format(num_layers)
                saver.save(sess,'../Model/model/'+model_name+str(epoch)+'.ckpt')
